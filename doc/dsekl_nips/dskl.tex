...
\begin{figure}[!ht]
	\begin{subfigure}[b]{0.45\textwidth}
		...
		\caption{\centering Validation error on the full covertype data set after computing $\mathcal{I}^{(k)}$. The full dataset and kernel map is evaluated after 58 steps.}
...
Figure~\ref{fig:covertype_validation} depicts the validation error after evaluation of the next predicate batch $\mathcal{I}^{(k)}$, one epoch amounts to 58 predicate batch evaluations. Note, that after one epoch the validation error decreases drastically from 51\% to ca. 17\%. We separated a random sample of 20,000 data points as test set from the training data. We achieve a final error rate on the test set after 54 epochs of 13.34\%. 
