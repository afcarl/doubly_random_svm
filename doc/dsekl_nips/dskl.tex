...
\begin{figure}[!ht]
	\begin{subfigure}[b]{0.45\textwidth}
		...
		\caption{\centering Validation error on the full covertype data set after computing predicate batch $\mathcal{I}^{(k)}$. The full dataset and kernel map is evaluated after 58 steps.}
...
Figure~\ref{fig:covertype_validation} depicts the validation error after evaluation of the next predicate batch $\mathcal{I}^{(k)}$, one epoch amounts to 58 predicate batch evaluations. %Note, that after one epoch the validation error decreases drastically from 51\% to ca. 17\%. 
Note, that after less than 20 predicate batches the validation error decreases drastically from 49\% to ca. 19\%. We separated a random sample of 20,000 data points as test set from the training data. We achieve a final error rate on the test set after 54 epochs of 13.34\%. 
...
Figure~\autoref{fig:speedup_58} shows the speedup achieved through the usage of multiple cores in our shared-memory variant. A python implementation of algorithm~\autoref{alg:dskl_svm_distributed} runs on a 48 core machine with 500 GB main memory. We record the runtime for processing a single predicate batch, which is evaluated against all kernel batches in parallel. We duplicate the Covertype dataset use 4,770 samples for the kernel batches which amounts to 240 parallel processes and 31,250 samples for the predicate batches.
The parameters are chosen as such as a demonstration that the algorithm is scalable. However, due to the nature of python parallel processes -- which copy the data for each process --, with different parameters, a decrease in speedup might happen when using more parallel processes. We emphasize, however, that the algorithm per se lends itself naturally to parallelization and it should deliver better scaling performance if implemented in a programming environment, that allows for sharing memory between threads instead of copying it (e.g. Java or C++).
